...
public abstract class PlayerData implements AudioEventHandler {
  ...
  static final String STATUS_PAN_KEY_PATH = "pan";
  ...
  final void setStatusWithListener(final ReadableMap status, final SetStatusCompletionListener setStatusCompletionListener) {
    ...
    if (status.hasKey(STATUS_PAN_KEY_PATH)) {
      Log.d(TAG, "Set the pan key! from my custom version of the exoplayer");
      mPan = (float) status.getDouble(STATUS_PAN_KEY_PATH);
    }
  ...
  public synchronized final WritableMap getStatus() {
    ...
    map.putDouble(STATUS_PAN_KEY_PATH, mPan);
